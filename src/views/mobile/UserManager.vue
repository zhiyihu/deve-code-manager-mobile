            ...
            <div>
                <div v-for="(item,index) in data.slice(current*pageSize-pageSize,current*pageSize)" :key="index" class="table-info-part">
                    ...
                    <div class="table-info-optbtns">
                        ...
                        <a href="javascript:;" class="table-opt-alink" v-if="mypermission.includes('admin') && item.user != 'admin'||mypermission.includes('user-modify_regist_days') && item.user != uname" @click="modifyRegistDay(item);">修改注册天数</a>
